...
def to_bgr(image: carla.Image):
    ...
    # .copy() es esencial para evitar el error de "readonly" en OpenCV
    return arr[:, :, :3].copy()
...
def main():
    ...
    camera = world.spawn_actor(bp_lib.find("sensor.camera.rgb"), 
                               carla.Transform(carla.Location(x=1.6, z=1.7)), attach_to=vehicle)
    ...
    try:
        while running:
            ...
            if img is not None:
                frame = to_bgr(img) # Aquí ya tenemos la copia editable
            ...
            if joystick:
                ...
                # CÁLCULO DE STEER: Exponente + Limitador
