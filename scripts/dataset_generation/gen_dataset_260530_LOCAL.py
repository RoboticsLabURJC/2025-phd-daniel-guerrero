...
def process_image(image):
    ...
    image_queue.put((image.frame, image_rgb))
...
def main():
    ...
    csv_writer.writerow(['frame_global', 'throttle', 'steering', 'image_path', 'source_log'])
    ...
    try:
        for log_idx, log_path in enumerate(log_files, 1):
            ...
            while True:
                if not image_queue.empty():
                    frame_id, image_rgb = image_queue.get()
            ...
            # Limpieza local sin paréntesis en is_alive
        ...
        # Limpieza global sin paréntesis en is_alive
        if 'camera' in locals() and camera.is_alive:
            camera.stop()
            camera.destroy()
