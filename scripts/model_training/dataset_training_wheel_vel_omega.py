...
class CarlaDataset(Dataset):
    def __init__(self, csv_path, img_root_dir):
        ...
        required = {'frame', 'v_mps', 'omega_rps'}
        ...
        self.labels = self.data[['v_mps', 'omega_rps']].astype('float32').to_numpy()
    ...
    def __getitem__(self, idx):
        ...
        label = torch.from_numpy(self.labels[idx])  # (2,)
        return img, label
...
class CarlaMobileNet(nn.Module):
    def __init__(self):
        ...
        # Head para regresión (v, omega)
        self.base.classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.base.last_channel, 2)  # v_mps, omega_rps
        )
...
if __name__ == "__main__":
    csv_path = "dataset_ego/controls.csv"
    img_root_dir = "dataset_ego/images"

    device = torch.device("cpu")  # Cambia a "cuda" o "cuda:0" si ROCm/NVIDIA aparece como cuda
    ...
    torch.save(model, "carla_model_mobilenet_v_omega.pth")
    print("✅ Modelo guardado como carla_model_mobilenet_v_omega.pth")
